...
fs.readFile(jishujuURL,(err,docs)=>{
    ...
    arr.forEach((item,i)=>{
        ...
        arr[i].buydate = Date.parse(new Date()) -  Random.integer(0,365 * 24 * 60 *60 *1000 * 20);
        // license  是否带牌买
        arr[i].license = Random.boolean();
        // exhaust  排放标准
        arr[i].exhaust = Random.pick(["国一","国二","国三","国四","国五"]);
        //gearbox 变速箱
        arr[i].gearbox = Random.pick(["手动","自动","手自一体"]);
        //fuel
        arr[i].fuel = Random.pick(["汽油","柴油","油电混合","天然气","纯电动"]);
